package com.sulake.habbo.catalog.viewer.widgets
{
   ...
   public class ColourGridCatalogWidget extends CatalogWidget implements ICatalogWidget
   {
      ...
      private var var_338:IItemGridWindow;
      
      private var var_1574:XML;
      ...
      override public function init() : Boolean
      {
         ...
         this.var_338 = _window.findChildByName("colourGrid") as IItemGridWindow;
         var _loc1_:XmlAsset = page.viewer.catalog.assets.getAssetByName("color_chooser_cell") as XmlAsset;
         this.var_1574 = _loc1_.content as XML;
      ...
      private function onAvailableColours(param1:CatalogWidgetColoursEvent) : void
      {
         ...
         this.select(this.var_338.getGridItemAt(0) as IWindowContainer);
      }
      ...
      private function onAvailableMultiColours(param1:CatalogWidgetMultiColoursEvent) : void
      {
         ...
         for each(_loc2_ in param1.colours)
         {
            this.var_854.push(_loc2_.slice());
         }
         _loc3_ = page.viewer.catalog.assets.getAssetByName(param1.backgroundAssetName) as BitmapDataAsset;
         this.var_563 = _loc3_.content as BitmapData;
         _loc4_ = page.viewer.catalog.assets.getAssetByName(param1.colourAssetName) as BitmapDataAsset;
      ...
      private function select(param1:IWindowContainer) : void
      {
         ...
         {
            _loc2_ = this.var_384.getChildByName("chosen");
            if(_loc2_ != null)
            {
      ...
      private function populateColourGrid() : void
      {
         ...
         var _loc12_:int = 0;
         var _loc13_:int = 0;
         Logger.log("[ColourGridCatalogWidget] Display colors: " + [this.var_854.length,this.var_338.numGridItems]);
         this.var_338.destroyGridItems();
         this.var_384 = null;
         for each(_loc1_ in this.var_854)
         {
            if(_loc1_.length > 0)
            {
               _loc2_ = page.viewer.catalog.windowManager.buildFromXML(this.var_1574) as IWindowContainer;
               _loc2_.addEventListener(WindowMouseEvent.WINDOW_EVENT_MOUSE_CLICK,this.onClick);
               _loc2_.background = true;
               _loc2_.color = 4294967295;
               _loc2_.width = this.var_563.width;
               _loc2_.height = this.var_563.height;
               this.var_338.addGridItem(_loc2_);
               _loc3_ = _loc2_.findChildByTag("BG_BORDER") as IBitmapWrapperWindow;
               if(_loc3_ != null)
               {
                  _loc3_.bitmap = new BitmapData(this.var_563.width,this.var_563.height,true,0);
                  _loc3_.bitmap.copyPixels(this.var_563,this.var_563.rect,new Point(0,0));
               }
      ...
      private function onClick(param1:WindowMouseEvent) : void
      {
         ...
         var _loc2_:int = this.var_338.getGridItemIndex(param1.target as IWindow);
