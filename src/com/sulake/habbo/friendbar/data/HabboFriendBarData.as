package com.sulake.habbo.friendbar.data
{
   ...
   public class HabboFriendBarData extends Component implements IHabboFriendBarData
   {
      ...
      private static const const_330:Boolean = false;
      
      private static const const_967:String = "FriendBar";
      
      private static const const_966:String = "user_interaction";
      
      private static const const_1356:String = "visit_btn_click";
      
      private static const const_1355:String = "chat_btn_click";
       
      
      private var var_965:IHabboCommunicationManager;
      
      private var var_2561:IHabboMessenger;
      ...
      {
         return this.var_429.getValue(param1);
      }
      
      public function getFriendByName(param1:String) : IFriendEntity
      {
         var _loc2_:* = null;
         for each(_loc2_ in this.var_166)
         {
      ...
      {
         this.var_965.getHabboMainConnection(null).send(new FollowFriendMessageComposer(param1));
         this.var_965.getHabboMainConnection(null).send(new ConversionPointMessageComposer(const_967,const_966,const_1356));
      }
      
      public function startConversation(param1:int) : void
      {
         this.var_2561.startConversation(param1);
         this.var_965.getHabboMainConnection(null).send(new ConversionPointMessageComposer(const_967,const_966,const_1355));
      }
      
      private function onCommunicationManagerAvailable(param1:IID, param2:IUnknown) : void
      {
         this.var_965 = param2 as IHabboCommunicationManager;
         this.var_965.addHabboConnectionMessageEvent(new MessengerInitEvent(this.onMessengerInitialized));
         this.var_965.addHabboConnectionMessageEvent(new FriendListUpdateEvent(this.onFriendListUpdate));
      }
      
      private function onMessengerComponentAvailable(param1:IID, param2:IUnknown) : void
      {
         this.var_2561 = param2 as IHabboMessenger;
      }
      ...
      private function onFriendListUpdate(param1:IMessageEvent) : void
      {
         ...
         {
            _loc3_ = this.var_429.getValue(_loc8_);
            if(_loc3_)
            {
               this.var_429.remove(_loc8_);
               this.var_166.splice(this.var_166.indexOf(_loc3_),1);
            }
         }
         for each(_loc4_ in _loc6_)
         {
            _loc3_ = this.var_429.getValue(_loc4_.id);
            if(_loc3_)
            {
               if(_loc4_.online || const_330)
               {
               ...
               {
                  this.var_429.remove(_loc4_.id);
                  this.var_166.splice(this.var_166.indexOf(_loc3_),1);
               }
            }
            else if(_loc4_.online || const_330)
            {
               ...
               this.var_166.push(_loc3_);
               this.var_429.add(_loc3_.id,_loc3_);
            }
         }
         for each(_loc4_ in _loc7_)
         {
            if(this.var_429.getValue(_loc4_.id) == null)
            {
               _loc3_ = new FriendEntity(_loc4_.id,_loc4_.name,_loc4_.realName,_loc4_.motto,_loc4_.gender,_loc4_.online,_loc4_.followingAllowed,_loc4_.figure,_loc4_.categoryId,_loc4_.lastAccess);
               this.var_166.push(_loc3_);
               this.var_429.add(_loc3_.id,_loc3_);
         ...
         {
            this.var_166 = !!const_330 ? this.sortByNameAndOnlineStatus(this.var_166) : this.sortByName(this.var_166);
         }
         events.dispatchEvent(new FriendBarUpdateEvent(FriendBarUpdateEvent.const_529));
      }
