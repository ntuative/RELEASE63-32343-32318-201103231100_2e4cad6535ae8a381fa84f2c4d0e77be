package com.sulake.core.utils
{
   ...
   public class LibraryLoader extends EventDispatcher implements IDisposable
   {
      ...
      protected static const const_1733:uint = 1;
      
      protected static const const_1735:uint = 2;
      
      protected static const const_1734:uint = 4;
      
      protected static const const_1732:uint = 3;
      
      protected static const name_3:uint = 7;
      
      public static const const_1719:int = 5;
      
      public static const LIBRARY_LOADER_FINALIZE:String = "LIBRARY_LOADER_FINALIZE";
      
      public static const const_1158:Boolean = false;
      
      public static const const_1655:int = 2;
      ...
      protected var var_236:URLRequest;
      
      protected var _manifest:XML;
      
      protected var var_565:Class;
      
      protected var var_462:uint = 0;
      
      protected var _name:String;
      
      protected var var_1040:Boolean = false;
      
      protected var _debug:Boolean = false;
      
      protected var var_1041:Boolean = false;
      
      protected var var_2833:String = "";
      
      protected var var_2831:String = "";
      
      protected var var_2060:int;
      
      protected var var_1291:int;
      
      protected var var_1290:int;
      
      protected var var_2918:uint = 0;
      
      protected var var_2832:Boolean = true;
      ...
      protected static function queue(param1:LibraryLoader) : void
      {
         ...
         {
            param1.var_43.load(param1.var_236,param1._context);
         }
      }
      ...
      protected static function throttle() : void
      {
         ...
         if(const_1158)
         {
            ...
            while(var_705.length < const_1655 && false)
            {
               _loc1_ = var_878.shift() as LibraryLoader;
               if(!_loc1_.ready && !_loc1_.disposed)
               {
                  var_705.push(_loc1_);
                  _loc1_.var_43.load(_loc1_.var_236,_loc1_._context);
               }
            }
         }
      }
      ...
      {
         return !!this.var_236 ? this.var_236.url : null;
      }
      ...
      {
         return !!this.var_1040 ? uint(this.var_1291 - this.var_2060) : uint(getTimer() - this.var_2060);
      }
      ...
      override public function dispose() : void
      {
         if(!disposed)
         {
            dispatchEvent(new LibraryLoaderEvent(LibraryLoaderEvent.LIBRARY_LOADER_EVENT_DISPOSE,this.var_396,this.bytesTotal,this.bytesLoaded,this.elapsedTime));
            try
            {
               this.var_43.close();
            }
      ...
      public function load(param1:URLRequest, param2:int = 5) : void
      {
         ...
         this._name = parseNameFromUrl(this.var_236.url);
      ...
      {
         if(this.var_1041 && !_disposed)
         {
            this.var_1041 = false;
            if(!this.var_1040 && this.var_236)
            {
               this.var_1291 = -1;
               this.var_2060 = getTimer();
               queue(this);
            }
         }
      }
      ...
      {
         if(!this.var_1040 && !_disposed && this.var_1290 > 0)
         {
            try
            {
               this.var_43.close();
            ...
            this.addRequestCounterToUrlRequest(this.var_236,const_1719 - this.var_1290);
            if(this.var_2832)
            {
               this.var_236.requestHeaders.push(new URLRequestHeader("pragma","no-cache"));
               this.var_236.requestHeaders.push(new URLRequestHeader("Cache-Control","no-cache"));
               this.var_2832 = false;
            }
            --this.var_1290;
            this.var_462 = 0;
            this.var_43.load(this.var_236,this._context);
            return true;
         }
         return false;
      }
      ...
      protected function loadEventHandler(param1:Event) : void
      {
         ...
         switch(param1.type)
         {
            ...
            case HTTPStatusEvent.HTTP_STATUS:
               this.var_396 = HTTPStatusEvent(param1).status;
               this.debug("Load event STATUS " + this.var_396 + " for file \"" + this.url + "\"");
               break;
            case Event.UNLOAD:
               this.debug("Load event UNLOAD for file \"" + this.url + "\"");
               dispatchEvent(new LibraryLoaderEvent(LibraryLoaderEvent.LIBRARY_LOADER_EVENT_UNLOAD,this.var_396,this.bytesTotal,this.bytesLoaded,this.elapsedTime));
               break;
            case ProgressEvent.PROGRESS:
               this.debug("Load event PROGRESS for file \"" + this.url + "\"  bytes: " + this.bytesLoaded + "/" + this.bytesTotal);
               dispatchEvent(new LibraryLoaderEvent(LibraryLoaderEvent.LIBRARY_LOADER_EVENT_PROGRESS,this.var_396,this.bytesTotal,this.bytesLoaded,this.elapsedTime));
               break;
            case IOErrorEvent.IO_ERROR:
               this.debug("Load event IO ERROR for file \"" + this.url + "\"");
               if(!this.handleHttpStatus(this.var_396))
               {
                  this.var_1291 = getTimer();
            ...
            case TimerEvent.TIMER:
               ...
               if(!_disposed)
               {
                  this.var_462 = LibraryLoader.name_3;
         ...
         if(this.var_462 == LibraryLoader.const_1732)
         {
         ...
         if(this.var_462 == LibraryLoader.name_3)
         {
            if(this.prepareLibrary())
            {
               ...
               this.removeEventListeners();
               throttle();
               dispatchEvent(new LibraryLoaderEvent(LibraryLoaderEvent.LIBRARY_LOADER_EVENT_COMPLETE,this.var_396,this.bytesTotal,this.bytesLoaded,this.elapsedTime));
      ...
      protected function prepareLibrary() : Boolean
      {
         ...
         this.var_565 = this.getDefinition(this._name) as Class;
         if(this.var_565 == null)
         {
            if(!this.retry())
            {
               this.failure("Failed to find resource class \"" + this._name + "\" from library " + this.var_236.url + "!");
            }
            return false;
         }
         try
         {
            xmlClass = this.var_565.manifest as Class;
            if(xmlClass == null)
            {
               return false;
            }
      ...
      protected function debug(param1:String) : void
      {
         ...
         if(this._debug)
         {
            dispatchEvent(new LibraryLoaderEvent(LibraryLoaderEvent.LIBRARY_LOADER_EVENT_DEBUG,this.var_396,this.bytesTotal,this.bytesLoaded,this.elapsedTime));
         }
      }
      ...
      protected function failure(param1:String) : void
      {
         ...
         throttle();
         dispatchEvent(new LibraryLoaderEvent(LibraryLoaderEvent.LIBRARY_LOADER_EVENT_ERROR,this.var_396,this.bytesTotal,this.bytesLoaded,this.elapsedTime));
         dispatchEvent(new Event(LIBRARY_LOADER_FINALIZE));
      }
